...
//#define cDefaultCP	CP_ACP
#define cDefaultCP	1251
...
#ifdef _UNICODE
	MultiByteToWideChar(cDefaultCP, 0, (char*)szBuff, -1, pszBuff, 31);
#else
...
BOOL CopyTagStringW(BYTE *buf, int buflen, LPWSTR str, int strlen)
{
	...
	if (buf[0] == 0) {
		...
		MultiByteToWideChar(cDefaultCP, 0, psz, -1, str, strlen);
		str [strlen - 1] = NULL;
		ret = TRUE;
	}
...
BOOL CopyTagString(BYTE *buf, int buflen, LPTSTR str, int strlen)
{
	...
	WideCharToMultiByte(cDefaultCP, 0, szTemp, -1, str, strlen, NULL, NULL);
	str[strlen - 1] = NULL;
	return TRUE;
#endif
}
